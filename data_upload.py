...
from food_costs.models import Eggs, Flour, Sugar, Pepper, SaltedMackerel, SaltedShrimp, Anchovy, Squid, Rice, ChapRice, WhiteBeans, RedBeans, FlourFood, SugarFood
...
CSV_PATH2 = ['./datas/jajangmean_txt.csv','./datas/nangmean_txt.csv' ]
path2 = [FlourFood, SugarFood]
k = 0
for i in CSV_PATH2:

  with open(i, 'r',  encoding='utf-8') as file:
        # csv.reader(파일식별자)
        data_rows2 = csv.reader(file, skipinitialspace=True)
        # header(첫번째 줄) 제외. 해당 코드 문구 하나마다 한 행을 스킵.
        next(data_rows2, None)
        ...
        # 데이터 중에 비어있는 줄 없애기(filter로 None값 삭제)
        data_rows2 = filter(None, list(data_rows2))
        ...
        for row in data_rows2:
            ...
            if row[0] != None or row[0] != '':
              ...
              # 중복 회피, 업로딩 => update_or_create 메소드 사용
              path2[k].objects.update_or_create(
                # filter : 중복을 체크할 값
                # 이하 코드 줄이 update_or_create 기능을 시행하는 것.
                date = row[0], 
